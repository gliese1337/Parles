...
from ParlesAST import Sequence, Quote, Scope, Word, Func, Pop
...
def _merge(node):
  if isinstance(node, Sequence):
    ...
    return node.__class__(nleft, right), args
...
def merge(node):
  if isinstance(node, Sequence):
    ...
    return node.__class__(nleft, nright)
  if isinstance(node, Scope):
    args, body = node
    nnode, nargs = _merge(body)
    if nnode == body: return node
    return node.__class__(args+nargs, nnode)
  return node

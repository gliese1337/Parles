...
from ParlesAST import Quote, Paren, Block, Word, Literal, Seq, Method, Pipe
from ParlesTypes import TypeVar, TypeName, StackType, AtomType, FuncType
...
def args_body(tokenizer):
  ...
  if type == 'ARG_ARROW':
    ...
    args = []
  return args, body, type, val

def parseQuote(tokenizer):
  args, body, type, val = args_body(tokenizer)
  if type != 'CLOSE_QUOTE':
...
def parseFunc(tokenizer):
  t1, (type, _) = parseStack(tokenizer)
  if type != 'ARG_ARROW':
    raise Exception("Invalid type declaration")
  t2, (type, _) = parseStack(tokenizer)
  if type != 'CLOSE_PAREN':
    raise Exception("Invalid type declaration")
...
def parseLine(tokenizer, next=None, stops=[]):
  ...
  line = []
  if next is None: next = tokenizer.next()
  while True:
    type, val = next
